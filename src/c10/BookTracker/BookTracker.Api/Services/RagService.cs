...
public class RagService : IRagService
{
    ...
    public async Task<RecommendResult> RecommendAsync(string query, CancellationToken ct = default)
    {
        ...
        var response = await _client.Messages.Create(new MessageCreateParams
        {
            ...
            // Wrap the untrusted query as data; the retrieved context stays in the cached system block.
            Messages = [new() { Role = Role.User, Content = PromptSafety.Wrap(query) }],
        ...
        return new RecommendResult(
            reply, sources, (int)response.Usage.InputTokens, (int)response.Usage.OutputTokens);
    }
}
